...
export default function AnalyticsEnhanced() {
  ...
  const {
    ...
    isLoading,
    refetch,
  } = trpc.invoices.getAnalytics.useQuery(
    { timeRange },
  ...
  const { data: clientProfitability } =
    trpc.analytics.getClientProfitability.useQuery(undefined, {
      enabled: isAuthenticated,
    });

  const { data: cashFlowProjection } =
    trpc.analytics.getCashFlowProjection.useQuery(
      { months: 6 },
      { enabled: isAuthenticated }
    );

  const { data: revenueVsExpenses } =
    trpc.analytics.getRevenueVsExpenses.useQuery(
  ...
  const netProfit =
    parseFloat(analytics?.totalRevenue?.toString() || "0") -
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Expected income and expenses for next 6 months
                </CardDescription>
              </CardHeader>
              <CardContent>
                ...
                <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
                  ...
                  <div className="p-4 rounded-lg border border-yellow-200 dark:border-yellow-800 bg-yellow-50 dark:bg-yellow-950/20">
                    ...
                    <div className="text-2xl font-bold text-yellow-600 dark:text-yellow-400">
                      {agingReport.days_0_30.count}
                    </div>
                    <div className="text-xs text-yellow-600 dark:text-yellow-400 mt-1">
                      {formatCurrency(agingReport.days_0_30.amount)}
                    </div>
                  </div>
                  <div className="p-4 rounded-lg border border-orange-200 dark:border-orange-800 bg-orange-50 dark:bg-orange-950/20">
                    ...
                    <div className="text-xs text-orange-600 dark:text-orange-400 mt-1">
                      {formatCurrency(agingReport.days_31_60.amount)}
                    </div>
                  </div>
                  <div className="p-4 rounded-lg border border-destructive/30 bg-destructive/10">
                    ...
                    <div className="text-xs text-destructive dark:text-destructive mt-1">
                      {formatCurrency(agingReport.days_61_90.amount)}
                    </div>
                  </div>
                  <div className="p-4 rounded-lg border border-destructive bg-destructive/20">
                    ...
                    <div className="text-2xl font-bold text-red-700">
                      {agingReport.days_90_plus.count}
                    </div>
                    <div className="text-xs text-red-700 mt-1 font-semibold">
                      {formatCurrency(agingReport.days_90_plus.amount)}
                    </div>
                  </div>
                </div>
              </CardContent>
                    ...
                    <tbody>
                      {clientProfitability.slice(0, 10).map((client: any) => (
                        ...
                        >
                          ...
                          <td className="text-right p-3 text-destructive dark:text-destructive">
          ...
          {revenueVsExpenses && revenueVsExpenses.length > 0 && (
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Revenue vs expenses for {new Date().getFullYear()}
                </CardDescription>
              </CardHeader>
              <CardContent>
                <ResponsiveContainer width="100%" height={300}>
