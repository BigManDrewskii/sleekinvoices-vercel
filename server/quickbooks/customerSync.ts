...
function clientToQBCustomer(client: any): Partial<QBCustomer> {
  ...
  const customer: any = {
    ...
    CompanyName: client.companyName?.substring(0, 100),
    Active: true,
  };
...
export async function getCustomerMapping(
  userId: number,
  clientId: number
): Promise<{ qbCustomerId: string; syncVersion: number } | null> {
  const db = await getDb();
  if (!db) return null;
  const mappings = await db
    ...
    .where(
      and(
        eq(quickbooksCustomerMapping.userId, userId),
        eq(quickbooksCustomerMapping.clientId, clientId)
      )
    )
    .limit(1);
  if (!mappings || mappings.length === 0 || !mappings[0]) return null;
  return {
    qbCustomerId: mappings[0].qbCustomerId,
...
export async function syncClientToQB(
  ...
  try {
    if (existingMapping) {
      const existingCustomer = await getQBEntity<QBCustomer>(
        userId,
        "Customer",
        existingMapping.qbCustomerId
      );
      if (!existingCustomer.success || !existingCustomer.data)
        return {
          success: false,
          error: "Failed to fetch existing customer from QuickBooks",
        };

      const updateData = {
        ...customerData,
        Id: existingMapping.qbCustomerId,
        SyncToken: existingCustomer.data.SyncToken,
        sparse: true,
      };
      const result = await updateQBEntity<QBCustomer>(
        userId,
        "Customer",
        updateData
      );
      ...
      await db
        .update(quickbooksCustomerMapping)
        .set({
      ...
      const existingQBCustomer = await findQBCustomer(
        userId,
      ...
      const result = await createQBEntity<QBCustomer>(
        userId,
        "Customer",
        customerData
      );
    ...
    await logSync(
      ...
      "customer",
      clientId,
      existingMapping?.qbCustomerId || null,
...
export async function syncAllClientsToQB(
  userId: number
...
export async function getClientSyncStatus(
  ...
  return {
    ...
    qbDisplayName: mapping.qbDisplayName,
