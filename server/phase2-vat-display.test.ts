...
describe('Phase 2: VAT Display and Decimal Integration', () => {
  ...
  beforeAll(async () => {
    ...
    const [clientWithVat] = await db.insert(clients).values({
      userId: testUserId,
    ...
    const [taxExemptClient] = await db.insert(clients).values({
      userId: testUserId,
  ...
  afterAll(async () => {
    ...
    // Cleanup test data
    await db.delete(invoiceLineItems).where(eq(invoiceLineItems.invoiceId, testClientId));
  ...
  describe('2.3 Clients List VAT Column', () => {
    ...
    it('2.3.2 should return taxExempt in client list query', async () => {
      ...
      const taxExemptClient = clientList.find(c => c.id === testTaxExemptClientId);
