...
describe("Client Management Enhancements Round 2", () => {
  describe("Client Search Filters", () => {
    ...
    it("should filter by date range", () => {
      ...
      expect(filtered).toHaveLength(2);
      expect(filtered.every(c => new Date(c.createdAt) >= cutoffDate)).toBe(
        true
      );
    });
    ...
    const presets = [
      {
        ...
        content:
          "Hi {{clientName}}, just a quick reminder about invoice {{invoiceNumber}}.",
      },
      {
        id: "formal",
        ...
        content:
          "Dear {{clientName}}, This is a reminder regarding invoice {{invoiceNumber}}.",
      },
      {
        id: "urgent",
        ...
        content:
          "Dear {{clientName}}, URGENT: Invoice {{invoiceNumber}} requires immediate payment.",
      },
    ];
  ...
  describe("Export Clients to CSV", () => {
    ...
    it("should generate valid CSV content", () => {
      ...
      expect(csvContent).toContain(
        'John Smith,john@example.com,"Acme, Inc.",No'
      );
