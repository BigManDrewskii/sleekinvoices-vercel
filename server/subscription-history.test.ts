...
describe('Subscription History API', () => {
  describe('History item transformation', () => {
    ...
    it('should transform crypto payment correctly', () => {
      ...
      expect(result.amount).toBe(28.50);
      expect(result.currency).toBe('USD');
      expect(result.cryptoCurrency).toBe('USDT_BSC');
      expect(result.cryptoAmount).toBe(28.50);
  ...
  describe('Summary calculations', () => {
    it('should calculate total spent from completed payments', () => {
      const items = [
        { status: 'finished', amount: 28.50 },
        { status: 'waiting', amount: 10.00 },
        { status: 'finished', amount: 102.00 },
        { status: 'failed', amount: 54.00 },
      ];
      ...
      expect(totalSpent).toBe(130.50); // 28.50 + 102.00
    });
