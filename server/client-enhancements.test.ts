...
describe('Client Management Enhancements', () => {
  describe('Client Sorting', () => {
    it('should sort clients by name ascending', () => {
      const clients = [
        { id: 1, name: 'Zebra Corp', email: 'z@test.com', createdAt: new Date('2026-01-01') },
        { id: 2, name: 'Alpha Inc', email: 'a@test.com', createdAt: new Date('2026-01-02') },
        { id: 3, name: 'Beta LLC', email: 'b@test.com', createdAt: new Date('2026-01-03') },
      ];
  ...
  describe('Placeholder Textarea', () => {
    it('should identify valid placeholders', () => {
      const validPlaceholders = ['clientName', 'invoiceNumber', 'invoiceAmount', 'dueDate', 'daysOverdue', 'invoiceUrl', 'companyName'];
      const template = 'Dear {{clientName}}, your invoice {{invoiceNumber}} is overdue.';
      
      const foundPlaceholders = template.match(/\{\{(\w+)\}\}/g) || [];
      const extractedKeys = foundPlaceholders.map(p => p.replace(/\{\{|\}\}/g, ''));
      
      expect(extractedKeys).toContain('clientName');
      expect(extractedKeys).toContain('invoiceNumber');
      expect(extractedKeys.every(key => validPlaceholders.includes(key))).toBe(true);
    });
    ...
    it('should identify invalid placeholders', () => {
      ...
      const invalidKeys = extractedKeys.filter(key => !validPlaceholders.includes(key));
      
      expect(invalidKeys).toContain('unknownField');
    });

    it('should replace placeholders with sample values', () => {
      const template = 'Dear {{clientName}}, your invoice {{invoiceNumber}} for {{invoiceAmount}} is due.';
      ...
      expect(preview).toBe('Dear John Smith, your invoice INV-2026-001 for $1,250.00 is due.');
    });
