...
// Sample data
const clientData = [
  { name: 'Acme Corporation', email: 'billing@acme.com', companyName: 'Acme Corp', address: '123 Business Ave, New York, NY 10001', phone: '+1-555-0101' },
  { name: 'TechStart Inc', email: 'accounts@techstart.io', companyName: 'TechStart Inc', address: '456 Innovation Blvd, San Francisco, CA 94102', phone: '+1-555-0102' },
  { name: 'Global Designs LLC', email: 'finance@globaldesigns.co', companyName: 'Global Designs LLC', address: '789 Creative St, Austin, TX 78701', phone: '+1-555-0103' },
  { name: 'Summit Consulting', email: 'ap@summitconsulting.com', companyName: 'Summit Consulting Group', address: '321 Advisory Lane, Chicago, IL 60601', phone: '+1-555-0104' },
  { name: 'Riverside Media', email: 'invoices@riversidemedia.net', companyName: 'Riverside Media Group', address: '654 Content Way, Los Angeles, CA 90001', phone: '+1-555-0105' },
];

const productData = [
  { name: 'Web Development', description: 'Full-stack web development services', rate: '150.00', unit: 'hour' },
  { name: 'UI/UX Design', description: 'User interface and experience design', rate: '125.00', unit: 'hour' },
  { name: 'Mobile App Development', description: 'iOS and Android app development', rate: '175.00', unit: 'hour' },
  { name: 'SEO Optimization', description: 'Search engine optimization services', rate: '500.00', unit: 'project' },
  { name: 'Content Writing', description: 'Professional content creation', rate: '75.00', unit: 'hour' },
  { name: 'Logo Design', description: 'Brand logo design package', rate: '800.00', unit: 'project' },
  { name: 'Social Media Management', description: 'Monthly social media management', rate: '1500.00', unit: 'month' },
  { name: 'Consulting Session', description: 'Business strategy consulting', rate: '200.00', unit: 'hour' },
  { name: 'Email Marketing Setup', description: 'Email campaign setup and automation', rate: '350.00', unit: 'project' },
  { name: 'Maintenance Retainer', description: 'Monthly website maintenance', rate: '500.00', unit: 'month' },
];
...
const expenseVendors = [
  'Adobe Creative Cloud', 'AWS', 'Google Workspace', 'Figma', 'Notion',
  'Office Depot', 'Staples', 'Amazon Business',
  'United Airlines', 'Uber', 'Hilton Hotels',
  'Google Ads', 'Facebook Ads', 'LinkedIn Ads',
  'Legal Firm LLC', 'Accounting Partners', 'Insurance Co'
];
...
async function seedDevData() {
  ...
  try {
    ...
    const [existingClients] = await connection.execute(
      'SELECT COUNT(*) as count FROM clients WHERE userId = ?', [userId]
    );
    if (existingClients[0].count > 0) {
      console.log('⚠️  User already has data. Use --force to overwrite (not implemented).');
    ...
    for (let i = 1; i <= 15; i++) {
      ...
      const status = invoiceStatuses[randomBetween(0, invoiceStatuses.length - 1)];
      ...
      let amountPaid = '0.00';
      if (status === 'paid') {
        paidAt = formatDate(new Date(dueDate.getTime() - randomBetween(1, 20) * 24 * 60 * 60 * 1000));
        amountPaid = total;
      }
      ...
      const [result] = await connection.execute(
        ...
        [userId, clientId, generateInvoiceNumber('INV', i), status, subtotal, taxRate, taxAmount, total, amountPaid,
         formatDate(issueDate), formatDate(dueDate), paidAt]
      );
    ...
    for (let i = 1; i <= 5; i++) {
      ...
      const [result] = await connection.execute(
        `INSERT INTO estimates (userId, clientId, estimateNumber, status, currency, subtotal, taxRate, taxAmount,
         ...
         VALUES (?, ?, ?, ?, 'USD', ?, ?, ?, 'percentage', '0', '0', ?, ?, ?, ?, NOW(), NOW())`,
        [userId, clientId, generateInvoiceNumber('EST', i), status, subtotal, taxRate, taxAmount, total,
         `Project Proposal ${i}`, formatDate(issueDate), formatDate(validUntil)]
      );
    ...
    for (let i = 0; i < 20; i++) {
      ...
      const vendor = expenseVendors[randomBetween(0, expenseVendors.length - 1)];
      ...
      const clientId = isBillable ? clientIds[randomBetween(0, clientIds.length - 1)] : null;
      ...
      await connection.execute(
        ...
        [userId, categoryId, amount, formatDate(date), `${vendor} - Monthly subscription`, vendor,
         paymentMethod, isBillable, clientId]
      );
    }
